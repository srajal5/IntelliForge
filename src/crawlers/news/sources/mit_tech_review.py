"""MIT Technology Review AI RSS news source."""
...
class MITTechReviewSource(BaseNewsSource):
    """Discovers news articles from MIT Technology Review AI RSS feed."""
    ...
    async def discover(self, limit: int = 10, offset: int = 0) -> list[RawNewsArticle]:
        ...
        if not result.success or not result.content:
            logger.warning("mit_tech_review_fetch_failed", status=result.status_code)
            return discovered
        ...
        for article in selected:
            ...
            if not article.full_text or len(article.full_text) < 50:
                try:
                    ...
                    logger.debug("mit_tech_review_full_text_error", url=article.url, error=str(exc))
