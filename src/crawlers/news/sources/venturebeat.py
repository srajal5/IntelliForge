"""VentureBeat AI RSS news source."""
...
class VentureBeatSource(BaseNewsSource):
    """Discovers news articles from VentureBeat AI RSS feed."""
    ...
    async def discover(self, limit: int = 10, offset: int = 0) -> list[RawNewsArticle]:
        ...
        if not result.success or not result.content:
            logger.warning("venturebeat_fetch_failed", status=result.status_code)
            return discovered
        ...
        for article in selected:
            ...
            if not article.full_text or len(article.full_text) < 50:
                try:
                    ...
                    logger.debug("venturebeat_full_text_error", url=article.url, error=str(exc))
